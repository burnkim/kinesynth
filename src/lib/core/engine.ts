...
export function createEngine(
	...
	// 라우팅 캐시: 패치마다 대상 목록을 들고 있다가 rev가 바뀔 때만 다시 고른다.
	...
	function ctxOf(i: number): StepCtx {
		...
		if (sel === ALL) {
			...
			const cache = routes[i];
			if (cache.rev !== world.rev) {
				const tags = parseSelector(sel);
				cache.list = world.entities.filter((e) => matches(e, tags));
				cache.rev = world.rev;
			}
			ctx.targets = cache.list;
		}
		if (p.anchor) {
			const tags = parseSelector(p.anchor);
			ctx.anchor = world.entities.find((e) => matches(e, tags));
		}
		return ctx;
	}
