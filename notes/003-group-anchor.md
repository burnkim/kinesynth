...
- ~~도피가 아직 **전파되지 않는다**~~ → [004 · 공포는 개체보다 빨리 번진다](004-panic-wave.md)에서 해결.
  `Panic@flock`이 `sig.flee`를 읽어 이웃에게 넘긴다. 이 장면에도 얹혀 있는데
  **사냥의 리듬은 그대로다**(시도 3회로 동일) — 달라지는 건 무리가 반응하는 방식이다.
